...
class TestMeldCLI:
    ...
    def test_extend_meld_function(self, monkeypatch):
        ...
        monkeypatch.setattr(main, "select_meld_from_table", 
                           lambda gs: (0, self.table_meld))
        
        # Mock card selection to return the extension card
        monkeypatch.setattr(main, "select_cards_from_hand", 
                           lambda p, m: [extension_card])
    ...
    def test_handle_declaration_initial(self, monkeypatch):
        ...
        valid_sequence = Meld(
            cards=[high_value_cards[0], high_value_cards[1], high_value_cards[2]],
            type="sequence"
        )
        
        # Create a set meld
        valid_set = Meld(
            cards=[high_value_cards[3], high_value_cards[4], high_value_cards[5]],
            type="set"
        )
