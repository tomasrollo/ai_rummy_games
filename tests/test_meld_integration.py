...
class TestMeldIntegration:
    ...
    def test_declaration_extension_flow(self):
        ...
        p2_sequence = Meld(cards=[player2_cards[0], player2_cards[1], player2_cards[2]], type="sequence")

        # This should fail normally due to not enough points
        declaration_result = self.game_state.validate_and_process_declaration(self.player2.name, [p2_sequence])
        ...
        p2_sequence = Meld(cards=[player2_cards[0], player2_cards[1], player2_cards[2]], type="sequence")
        p2_set = Meld(
            cards=[high_value_cards[1], high_value_cards[2], high_value_cards[3]], type="set"  # Valid set of Q's
        )
        ...
        for meld in self.game_state.melds_on_table:
            if meld.type == "sequence" and any(card.suit == "Diamonds" and card.rank == "7" for card in meld.cards):
        ...
        # Validate extension
        extension_valid = self.validator.validate_extension(sequence_to_extend, [extension_card])
